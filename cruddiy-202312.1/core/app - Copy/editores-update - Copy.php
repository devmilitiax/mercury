...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("UPDATE `editores` SET `id_grapesjs`=?,`nombre`=?,`contenido_grapesjs`=?,`contenido_html`=?,`filtro`=?,`indice`=?,`email`=?,`asunto`=?,`fecha`=? WHERE `id`=?");

        try {
            $stmt->execute([ $id_grapesjs, $nombre, $contenido_grapesjs, $contenido_html, $filtro, $indice, $email, $asunto, $fecha, $id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: editores-read.php?id=$id");
        } else {
...
if(isset($_GET["id"]) && !empty($_GET["id"])){
    ...
    $sql = "SELECT * FROM `editores` WHERE `id` = ?";
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_num_rows($result) == 1){
                ...
                $id_grapesjs = htmlspecialchars($row["id_grapesjs"] ?? "");
					...
					$contenido_grapesjs = htmlspecialchars($row["contenido_grapesjs"] ?? "");
					$contenido_html = htmlspecialchars($row["contenido_html"] ?? "");
...
<?php endif ?>
						...
						<div class="form-group">
                                            ...
                                            <textarea name="contenido_grapesjs" id="contenido_grapesjs" class="form-control"><?php echo @$contenido_grapesjs; ?></textarea>
                                        </div>
						<div class="form-group">
                        ...
                        <hr>
                        <p>
                            <a href="editores-read.php?id=<?php echo $_GET["id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
