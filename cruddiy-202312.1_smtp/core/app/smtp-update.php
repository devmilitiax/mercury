...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if (!in_array(true, array_column($upload_results, 'error'))) {
        ...
        $stmt = $link->prepare("UPDATE `smtp` SET `name`=?,`hostname`=?,`username`=?,`password`=?,`port`=?,`from_email`=? WHERE `id`=?");

        try {
            $stmt->execute([ $name, $hostname, $username, $password, $port, $from_email, $id  ]);
        } catch (Exception $e) {
        ...
        if (!isset($error)){
            header("location: smtp-read.php?id=$id");
        } else {
                ...
                <div class="col-md-6 mx-auto">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                                            ...
                                            <input type="text" name="name" id="name" maxlength="50" class="form-control" value="<?php echo @$name; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="hostname">hostname*</label>
                                            <input type="text" name="hostname" id="hostname" maxlength="500" class="form-control" value="<?php echo @$hostname; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="username">username*</label>
                                            <input type="text" name="username" id="username" maxlength="500" class="form-control" value="<?php echo @$username; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="password">password*</label>
                                            <input type="text" name="password" id="password" maxlength="500" class="form-control" value="<?php echo @$password; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="port">port*</label>
                                            <input type="text" name="port" id="port" maxlength="4" class="form-control" value="<?php echo @$port; ?>">
                                        </div>
						<div class="form-group">
                                            <label for="from_email">from_email*</label>
                                            <input type="text" name="from_email" id="from_email" maxlength="500" class="form-control" value="<?php echo @$from_email; ?>">
                                        </div>
                        ...
                        <hr>
                        <p>
                            <a href="smtp-read.php?id=<?php echo $_GET["id"];?>" class="btn btn-info"><?php translate('View Record') ?></a>
